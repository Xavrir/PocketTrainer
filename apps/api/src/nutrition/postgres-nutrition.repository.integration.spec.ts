...
if (!ownerUrl || !runtimeUrl) {
  ...
  describe('PostgresNutritionRepository integration', () => {
    ...
    const repository = new PostgresNutritionRepository(
      loadConfig({
        NODE_ENV: 'test',
        DATA_STORE: 'postgres',
    ...
    beforeAll(async () => {
      await owner.query('insert into users(id) values ($1),($2)', [
        userA,
        userB,
      ]);
      await owner.query(
      ...
      values ($1,null,'3017620422003','Global provider food',15,'g',80,1,9,4,'open_food_facts',true)`,
        [globalFoodId],
      );
    });
    ...
    afterAll(async () => {
      ...
      await runtime.end();
      await owner.query('delete from nutrition_foods where id=$1', [
        globalFoodId,
      ]);
    ...
    it('persists barcode foods per user and enforces entry ownership and daily totals', async () => {
      ...
      await expect(
        repository.findFoodByBarcode(userA, '8999999999993'),
      ).resolves.toMatchObject({ id: food.id });
      await expect(
        repository.findFoodByBarcode(userB, '8999999999993'),
      ).resolves.toBeNull();
      ...
      await expect(
        repository.getDailyNutrition(userA, '2026-07-17', 'Asia/Jakarta'),
      ...
      await expect(
        repository.createFoodEntry(userB, randomUUID(), {
          foodId: food.id,
          servings: 1,
          consumedAt: '2026-07-17T01:00:00.000Z',
          mealType: 'snack',
      ...
      try {
        ...
        (user_id,food_id,servings,consumed_at,meal_type) values ($1,$2,1,now(),'snack')`,
            [userB, food.id],
          ),
        ...
        await expect(
          client.query('select id from nutrition_foods where id=$1', [
            globalFoodId,
          ]),
        ).resolves.toMatchObject({ rowCount: 1 });
        await expect(
          client.query(
            "update nutrition_foods set name='tampered' where id=$1",
            [globalFoodId],
          ),
